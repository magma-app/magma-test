...
export default {
  ...
  methods: {
    ...
    createChangedTag(index, event) {
      ...
      tag.key = event ? event.target.value : this.tagsCopy[index].key;
    ...
    createTagTexts(string) {
      ...
      return string.split(regex).map(key => ({ key }));
    },
    ...
    performAddTags(tag, event, source) {
      ...
      tags = tags.filter(tag => tag.key.trim().length > 0);
    ...
    duplicateFilter(tag) {
      return this.isDuplicate
        ...
        : !this.tagsCopy.find(t => t.key === tag.key);
    },
    addTag(tag, source = 'new-tag-input') {
      ...
      const options = this.filteredAutocompleteItems.map(i => i.key);
      if (this.addOnlyFromAutocomplete && options.indexOf(tag.key) === -1) return;
    ...
    performSaveTag(index, event) {
      ...
      if (tag.key.trim().length === 0) return;
    ...
    saveTag(index, tag) {
      ...
      if (this.avoidAddingDuplicates) {
        ...
        const dup = this.isDuplicate ?
          ...
          tagsDiff.map(t => t.key).indexOf(inputTag.key) !== -1;
