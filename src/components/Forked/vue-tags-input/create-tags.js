...
const validateUserRules = (tag, validation) => {
  return validation.filter(val => {
    ...
    if (typeof val.rule === 'string') return !new RegExp(val.rule).test(key);

    if (val.rule instanceof RegExp) return !val.rule.test(key);
...
const createClasses = (tag, tags, validation = [], customDuplicateFn) => {
  ...
  const index = findIndex(tags, t => t.key === tag.key);
...
const createTag = (tag, ...rest) => {
  ...
  if (tag.key === undefined) tag = { key: tag };
